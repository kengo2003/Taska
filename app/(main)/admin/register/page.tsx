...
export default function BulkRegisterPage() {
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto bg-white min-h-screen">
      <h1 className="text-2xl font-bold mb-6 text-gray-800">CSVアップロード登録</h1>
      
      {/* ▼▼▼ 注意事項エリア (ここを追加しました) ▼▼▼ */}
      <div className="bg-yellow-50 border-l-4 border-yellow-400 p-4 mb-8 text-sm text-yellow-800">
        <h3 className="font-bold mb-2 text-base">⚠️ CSV作成時の重要なお知らせ</h3>
        <ul className="list-disc pl-5 space-y-1">
          <li>
            Excelで保存する際は、ファイルの種類で<b>「CSV UTF-8 (コンマ区切り) (*.csv)」</b>を選択してください。<br/>
            <span className="text-xs text-gray-500">※通常の「CSV (コンマ区切り)」だと文字化けしてエラーになる場合があります。</span>
          </li>
          <li>
            1行目はヘッダーとして無視されます。データは2行目から入力してください。
          </li>
          <li>
            パスワードは<b>「8文字以上」</b>かつ<b>「数字・大文字・小文字・記号」</b>をすべて含める必要があります。
          </li>
        </ul>
      </div>
      {/* ▲▲▲ 注意事項エリアここまで ▲▲▲ */}
      ...
      {logs.length > 0 && (
        <div className="mt-10">
          <h2 className="font-bold mb-3 text-xl border-b pb-2">実行結果レポート</h2>
          <div className="border rounded overflow-hidden shadow-sm">
            <table className="w-full text-sm text-left">
              ...
              <tbody>
                {logs.map((log, i) => (
                  <tr key={i} className={log.status === "Error" ? "bg-red-50" : "bg-white border-b last:border-0"}>
                    ...
                    <td className="p-3 text-gray-600">{log.msg || "登録完了"}</td>
                  </tr>
                ))}
              </tbody>
            </table>

...
// ChatHistoryDateGroupコンポーネントがあればインポート（なければ自作が必要）
// import ChatHistoryDateGroup from "./ChatHistoryDateGroup"; 
...
const ChatSidebarContent = ({
  ...
  // 日付でグループ化するロジック（簡易版）
  // ※本来はChatHistoryDateGroupコンポーネントに任せるのが綺麗です
  ...
  return (
    <aside className="w-64 shrink-0 border-r border-gray-200 flex flex-col md:flex">
      ...
      <div className="flex-1 overflow-y-auto px-4 scrollbar-thin pb-4">
        ...
        {sessions.length === 0 && (
          <div className="text-xs text-gray-500 text-center py-4">履歴はありません</div>
        )}
      </div>
    </aside>
  );

...
export const ChatInput = ({ 
  input, setInput, isLoading, onSend, selectedFiles, onFileSelect, onRemoveFile 
}: Props) => {
        ...
        <div className="mb-2 flex flex-wrap gap-2 animate-in fade-in slide-in-from-bottom-2 bg-gray-50 p-2 rounded-lg border border-gray-200">
          {selectedFiles.map((file, index) => (
            <div key={index} className="relative bg-white border border-gray-200 rounded-md p-1.5 flex items-center gap-2 shadow-sm pr-7">
              {file.type.startsWith('image/') ? <ImageIcon className="w-3 h-3 text-purple-500"/> : <Paperclip className="w-3 h-3 text-blue-500" />}
      ...
      <div className="relative flex items-center group">
        ...
        <div className="absolute right-4 flex items-center gap-2">
          {input || selectedFiles.length > 0 ? (
             <button onClick={onSend} disabled={isLoading} className="text-blue-600 hover:text-blue-700">
                <Send className="w-5 h-5" />
             </button>
          ) : (
             <button onClick={() => setInput('')} className={`text-gray-400 ${!input && 'hidden'}`}>
                 <X className="w-5 h-5" />
             </button>
          )}
        </div>
